...
float Random::RandomFloatInRange(float from, float to)
{
	std::uniform_real_distribution<float> distribution(from, to);
	return distribution(generator);
}

std::unordered_map<const char*, float> ScopedTimer::m_Timings;
std::unordered_map<const char*, unsigned int> NamedCounter::m_NamedCounts;
