	...
	std::unordered_map<const MeshWorldObject::Mesh*, std::vector<MeshInstancess>>& cameraRenderData)
{
	...
	unsigned int perThread = glm::ceil((float)numInstances / numThreads);

	int threadsInUse = 0;

	const glm::mat4 camVP = m_Camera->GetProjection() * m_Camera->GetTransform();

	for (unsigned int i = 0; i < numThreads; ++i)
	{
		const unsigned int perThread = glm::ceil((float)(numInstances - totalProcessed) / (numThreads - i));
		...
		auto& outDataForCamera = m_RenderDataBuffer.m_CameraRenderDataBuffer[i];
		auto& outDataForPointlights = m_RenderDataBuffer.m_PointlightsRenderDataBuffer[i];

		outDataForCamera.clear();
		outDataForPointlights.clear();

		m_RendererThreadPool[i].m_WorkerFunction = [&]()
		{
			for (unsigned int i = totalProcessed; i < totalProcessed + numIndicesToProcess; ++i)
			{
				...
				glm::mat4 camMv = m_Camera->GetTransform() * instanceTransform;

				for (const MeshWorldObject::Mesh* mesh : mwoi->GetMeshWorldObject()->m_Meshes)
				{
					if (mwoi->GetMobility() == WorldObjectInstance::Mobility::Dynamic)
					{
	...
	NUMBERED_NAMED_COUNTER("Culling Threads in use", threadsInUse);
}
...
void Renderer::Draw(const World& world, WorldPartitionManager& worldPartitionManager, const FPSCamera& camera)
{
	...
	for (const MeshWorldObjectInstance* mwoi : m_OcclusionCulledInstances)
	{
		for (const MeshWorldObject::Mesh* mesh : mwoi->GetMeshWorldObject()->m_Meshes)
		{
